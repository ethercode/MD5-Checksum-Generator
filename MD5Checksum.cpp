...
RETURNS:        AnsiString : the hexadecimal MD5 checksum for the specified file
ARGUMENTS:      AnsiString& strFilePath : the full pathname of the specified file
NOTES:          Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
                ...
                exception.
*****************************************************************************************/
/*AnsiString CMD5Checksum::GetMD5(const AnsiString& strFilePath)
{
...
*****************************************************************************************/
/*
AnsiString CMD5Checksum::GetMD5(CFile& File)
{
    try
    {
...
RETURNS:        AnsiString : the hexadecimal MD5 checksum for the specified data
...
NOTES:          Provides an interface to the CMD5Checksum class. Any data that can
                ...
                function. Typically, AnsiString and char arrays will be checksumed, 
...
*****************************************************************************************/
AnsiString CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
...
RETURNS:        AnsiString : the final hexadecimal MD5 checksum result 
ARGUMENTS:      None
...
AnsiString CMD5Checksum::Final()
{
    ...
    //Convert the hexadecimal checksum to a AnsiString
    AnsiString  strMD5;
    for (int i = 0; i < nMD5Size; i++)
    {
        ...
        {
            Str = Str.sprintf("0%X", lpszMD5[i]);
        }
        else
        {
            Str = Str.sprintf("%X", lpszMD5[i]);
        }
        strMD5 += Str;
    }
    return strMD5;
